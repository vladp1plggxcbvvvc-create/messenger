...
# Хранилище сигналов для звонков: room_id -> {"data": ..., "ts": ...}
signals = {}
...
@app.post("/api/signal")
def post_signal():
    """Принимает сигнал от одного браузера и кладёт его в очередь комнаты."""
    if not session.get("uid"):
        return jsonify(error="auth"), 401
    d = request.json or {}
    ...
    signals[room] = {"data": data, "ts": time.time()}
    return jsonify(ok=True)

@app.get("/api/signal/<room>")
def get_signal(room):
    """Отдаёт последний сигнал из комнаты и удаляет его."""
    if not session.get("uid"):
        return jsonify(error="auth"), 401
    # чистим старые (>2 минут)
    now = time.time()
    for k in list(signals.keys()):
        if now - signals[k].get("ts", 0) > 120:
            del signals[k]
    entry = signals.pop(room, None)
    return jsonify(data=entry["data"] if entry else None)
